...
while True:
    ...
    if inp == "exit": 
        ...
        print("""
        Command: "formItogProtacol": # это фунция делает итоговый протакол на основе всех результатов
        Command: "import":       #НЕ ИСПОЛЬЗУЕТСЯ , формирует стартовый протокол с местом для попыток каждой трассы.
        Command: "calcCvalific": #НЕ ИСПОЛЬЗУЕТСЯ, считает квалификацию , используется вместе с import, с другими не использовать.
        Command: "formFinal": #формирование стартового протакола финалов,
        Command: "calcFinal": #подсчёт финалов
        Command: "formStart": # замена import
        Command: "calcStart": #подсчёт квалификации, использовать вместе с formStart()
        """)